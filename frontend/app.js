...
async function loadReleaseManifest() {
  const apiUrl = `${window.location.protocol}//${window.location.hostname}:8000/v1/updates/check`;
  ...
  try {
    ...
    console.warn("update api unavailable, using local manifest", error);
  }

  const staticResponse = await fetch("./downloads/safeagent-release-manifest.json", {
    cache: "no-store"
  });
...
if (bundleDownloadEl) {
  bundleDownloadEl.href = `${window.location.protocol}//${window.location.hostname}:8000/v1/updates/bundle`;
}
