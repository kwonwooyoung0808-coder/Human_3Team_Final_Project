from src.engines.action_engine import ActionEngine, FALLBACK_BLOCK_MESSAGE
...
def test_block_action_removes_original_response():
    ...
    assert result.message == FALLBACK_BLOCK_MESSAGE
    assert result.delivered_response == FALLBACK_BLOCK_MESSAGE
...
def test_action_result_has_run_id():
    ...
    assert result.run_id == "unit-004"
    assert result.action_type == "PASS"
    assert result.status == "applied"
    assert result.message == "Response passed policy checks."
    assert result.delivered_response == "정상 응답"


# === <> 테스트 코드 실행 방법 <> ===
# $env:PYTHONPATH='.'; pytest tests/unit/test_action_engine.py